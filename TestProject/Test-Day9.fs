...
let ``Test Day 9 Test Permutations with Preamble`` () =
    let input = [1; 2; 3; 4]
    let preambleSize = 3
    Ok [(1, 2); (1, 3); (2, 3); (2, 4); (3, 4)] =! (input |> Day9.createTuples preambleSize |> (Result.map Seq.toList))

[<Fact>]
let ``Test Day 9 checkXMAS Test1`` () =
    let input = ({1 .. 25} |> Seq.toList) @ [26]
    let preambleSize = 25
    Ok [Some (1,25,26)] =! (input |> Day9.checkXMAS preambleSize |> (Result.map Seq.toList))

[<Fact>]    
let ``Test Day 9 checkXMAS Test2`` () =
    let input = ({1 .. 25} |> Seq.toList) @ [49]
    let preambleSize = 25
    Ok [Some (24,25,49)] =! (input |> Day9.checkXMAS preambleSize |> (Result.map Seq.toList))
...
let ``Test Day 9 checkXMAS2 Test1`` () =
    ...
    let preambleSize = 25
    Ok [Ok (1L,25L,26L)] =! (input |> Day9.checkXMAS2 preambleSize |> (Result.map Seq.toList))

[<Fact>]    
let ``Test Day 9 checkXMAS2 Test2`` () =
    let input = ({1L .. 25L} |> Seq.toList) @ [49L]
    let preambleSize = 25
    Ok [Ok (24L,25L,49L)] =! (input |> Day9.checkXMAS2 preambleSize |> (Result.map Seq.toList))

[<Fact>]
let ``Test Day 9 checkXMAS2 Test3`` () =
    let input = ({1L .. 25L} |> Seq.toList) @ [100L]
    let preambleSize = 25
    Ok [Error 100L] =! (input |> Day9.checkXMAS2 preambleSize |> (Result.map Seq.toList))
    
[<Fact>]
let ``Test Day 9 checkXMAS2 Test4`` () =
    let input = ({1L .. 25L} |> Seq.toList) @ [50L]
    let preambleSize = 25
    Ok [Error 50L] =! (input |> Day9.checkXMAS2 preambleSize |> (Result.map Seq.toList))
...
let ``Test Day 9 checkXMAS2 Test5`` () =
    ...
    let preambleSize = 5
    Ok [127L] =! (input
                 |> Day9.checkXMAS2 preambleSize
                 |> (Result.map (Seq.choose (function Ok _ -> None | Error e -> Some e)>> Seq.toList )))
    
[<Fact>]
let ``Test Day 9/1 Final`` () =
    let preambleSize = 25
    Ok 15690279L =! (readLines "Inputs/Day9.txt" |> Result.bind (Day9.puzzle1 preambleSize))