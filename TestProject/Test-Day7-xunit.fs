...
let ``Test Day 7/1 Final`` () =
    Assert.Equal(Ok 378, readLines "Inputs/Day7.txt" |> Result.map (Day7.puzzle1 "shiny gold"))
    
[<Fact>]
let ``Test Day 7/2 Test1`` () =
    Assert.Equal(32, input1 |> (Day7.puzzle2 "shiny gold"))
    
[<Fact>]
let ``Test Day 7/2 Test2`` () =
    Assert.Equal(126, input2 |> (Day7.puzzle2 "shiny gold"))
    
[<Fact>]
let ``Test Day 7/2 Final`` () =
    Assert.Equal(Ok 27526, readLines "Inputs/Day7.txt" |> Result.map (Day7.puzzle2 "shiny gold"))